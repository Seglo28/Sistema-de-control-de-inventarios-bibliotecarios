...
public class usuarioController extends HttpServlet {
        ...
        try {
            switch (action) {
                case "Insertar":
                    ...
                    break;
                case "Insertar2":
                    this.Insertar2(request, response);
                    break;
                case "Actualizar":
    ...
    protected void Insertar2(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        ...
        try {
            ...
            request.setAttribute("msg", msg);
            request.setAttribute("lista", lista);
            rd = request.getRequestDispatcher("AdministracionMostrarUsuarios.jsp");
            rd.forward(request, response);
            conn.desconectar();
        } catch (Exception e) {
            System.out.println("Error en el metodo INSERTAR en el CONTROLADOR" + e.getMessage());
    ...
    protected void Actualizar(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            System.out.println("Error en el metodo ACTUALIZAR en el CONTROLADOR" + e.getMessage());
        }
    }
    ...
    protected void MostrarUno(HttpServletRequest request, HttpServletResponse response, String id)
        ...
        try {
            ...
            List<usuario> lista = dao.MostrarUno(Integer.parseInt(id));
    ...
    protected void MostrarTodos(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            RequestDispatcher rd;
            request.setAttribute("lista", lista);
            rd = request.getRequestDispatcher("UsuariosMostrarUsuarios.jsp");
            rd.forward(request, response);
        } catch (Exception e) {
    ...
    protected void Loging(HttpServletRequest request, HttpServletResponse response, String correo, String contrasena) {
        ...
        try {
            Conexion conn = new Conexion();
            UsuarioDAO dao = new UsuarioDAO(conn);
            List<usuario> lista = dao.Loging(correo, contrasena, correo);
            /*--------------------------------------------------------------------------------------------------------*/
            ...
            if (lista.size() > 0) { //SIZE Returns the number of elements in this list.
                ...
                pagina = "/PrincipalUsuarios.jsp";
            } else {
                msg = "<center>El sistema no reconoce alguno - o todos - de los datos ingresados.</center>";
                pagina = "/IngresoUsuario.jsp";
            }
            /*--------------------------------------------------------------------------------------------------------*/
            RequestDispatcher rd;
            request.setAttribute("mensage", msg);
